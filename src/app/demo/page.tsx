...
export default function DemoPage() {
  ...
  async function bootPixel() {
    setIsBusy(true);
    const seed = [
      { role: 'user', content: 'Greet me in ONE warm sentence, then immediately ask Q1/5.' },
    ];
    try {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      if (nextQ > MAX_Q) {
        const brief =
          'A stunning modern landing page with vibrant colors, an engaging hero section, and smooth scroll animations.';
        setSummary(brief);
        pushMessage(
          'ai',
          `✅ Requirements captured!\n\n**Your Design Brief:**\n${brief}\n\nClick the button below to watch your website come to life!`
...
<body class="bg-slate-900 text-white font-sans antialiased min-h-screen flex flex-col items-center justify-center p-8 text-center">
  ...
  if (phase === 'preview') {
    const continueLabel = isFullyConfigured ? '⚡ Continue to Checkout' : '🔧 Back to Configurator';
              ...
              <div className="flex items-center gap-1.5">
                ...
                {streamingCode.length < generatedHtml.length ? (
                  <span className="text-[10px] text-purple-400 font-mono bg-purple-500/10 px-1.5 rounded border border-purple-500/20">
                    Writing{' '}
                    {Math.round((streamingCode.length / Math.max(generatedHtml.length, 1)) * 100)}%
                  </span>
                ) : (
              ...
              <div className="absolute inset-0 bg-obsidian/0 group-hover:bg-obsidian/40 transition-colors duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100 pointer-events-none">
                <div className="flex items-center gap-2 bg-slate-900/95 border border-amber-500/30 px-4 py-2 rounded-lg shadow-xl">
                  ...
                  <span className="text-xs text-amber-300 font-mono">
                    Purchase to access source code
                  </span>
                </div>
              </div>
            </div>
            ...
            >
              <div>
                ...
                <p className="text-amber-400/70 text-xs mt-0.5">
                  Gemini free tier limit hit. Auto-retry in {rateLimitCountdown}s — or switch to
                  Anthropic API for unlimited generation.
                </p>
              </div>
              <div className="text-2xl font-black text-amber-400 font-mono shrink-0">
