  ...
  return (
    <div className="w-full p-4 md:p-6 pb-2 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 relative overflow-hidden min-h-[180px] md:min-h-[220px]">
      <div className="absolute top-0 right-0 p-4 opacity-20 pointer-events-none">
        <h2 className="text-2xl md:text-4xl font-black font-heading text-white/10 uppercase tracking-widest">
          Pipeline
        </h2>
      </div>
      ...
      <div className="absolute top-3 left-4 md:left-6 flex items-center gap-2 z-10">
        ...
        <div className="w-20 h-1 bg-white/10 rounded-full overflow-hidden">
          <div
            className="h-full bg-cyber-cyan rounded-full transition-all duration-700"
      ...
      <div className="flex justify-between items-start relative z-10 mt-8 md:mt-12 px-2 md:px-8 overflow-x-auto">
        ...
        {stages.map((stage, index) => {
          ...
          return (
            <div
              key={stage.id}
              className="flex flex-col items-center gap-2 md:gap-4 relative group min-w-[60px] md:w-24 flex-shrink-0"
            >
              <motion.div
                ...
                className={cn(
                  'w-8 h-8 md:w-12 md:h-12 rounded-full flex items-center justify-center border-2 transition-all duration-500 z-10 relative bg-obsidian',
                  isCompleted
                    ? 'border-emerald-500 text-emerald-400 shadow-[0_0_15px_rgba(52,211,153,0.3)]'
                    : isActive
                      ? 'border-cyber-cyan text-cyber-cyan shadow-[0_0_20px_rgba(255,107,26,0.5)] scale-110'
                      : isGate
              ...
              >
                {isCompleted ? (
                  ...
                  <div className="w-2 h-2 md:w-3 md:h-3 bg-cyber-cyan rounded-full animate-pulse" />
                ) : (
