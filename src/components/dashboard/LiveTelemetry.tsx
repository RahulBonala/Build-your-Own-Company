...
const generateData = (prevData: DataPoint[]) => {
  ...
  const userGrowth =
    Math.random() > 0.25 ? Math.floor(Math.random() * 4) + 1 : -Math.floor(Math.random() * 2);
          ...
          <div className="flex items-center gap-2">
            <div
              className={cn(
                ...
                latest.load > 90 ? 'bg-rose-500' : 'bg-emerald-500'
              )}
            />
            <span
              className={cn(
                'text-xs font-mono',
                latest.load > 90 ? 'text-rose-400' : 'text-emerald-400'
              )}
            >
              CPU {latest.load}%
            </span>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <h4 className="text-rose-400 font-bold uppercase tracking-wider text-sm flex items-center gap-2">
                  Critical Alert
                  <span className="text-[10px] bg-rose-500/20 px-1 rounded text-rose-300">
                    Database at 90%
                  </span>
                </h4>
                <p className="text-slate-300 text-xs mt-1">
                  Shared node capacity limit reached ({latest.users}/500).
                </p>
              </div>
            </div>
