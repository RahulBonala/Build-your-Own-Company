...
export default function CommunicationHub() {
  ...
  const updates = useMemo(() => {
    ...
    if (selections.design) {
      ...
      items.push({
        id: 1,
        text: `Design module locked in: ${label} tier.`,
        time: 'Just now',
        icon: CheckCircle2,
    ...
    if (selections.database) {
      ...
      items.push({
        id: 2,
        text: `Database configured: ${label}.`,
        time: '2m ago',
        icon: CheckCircle2,
    ...
    if (selections.testing) {
      ...
      items.push({
        id: 3,
        text: `QA pipeline selected: ${label}.`,
        time: '5m ago',
        icon: CheckCircle2,
    ...
    if (items.length === 0) {
      items.push({
        id: 5,
        text: 'No modules selected yet — head to the configurator.',
        time: 'Now',
        icon: AlertCircle,
    ...
    if (items.length < 4) {
      items.push({
        id: 6,
        text: `Project "${idea?.slice(0, 30) || 'Untitled'}" created.`,
        time: 'Earlier',
        icon: MessageSquare,
