    ...
    <div className="relative group">
      ...
      >
        <div className="relative z-10">
          <div className="text-sm font-bold flex items-center gap-2">{data.label}</div>
          <div className="text-xs opacity-70 mt-1 font-mono">
            {data.price
              ? `₹${data.price.toLocaleString()}`
              : `Setup: ₹${(data.setup || 0).toLocaleString()}`}
          </div>
        </div>
        ...
        {/* Selection Glow */}
        {isSelected && <div className="absolute inset-0 bg-cyber-cyan/5 blur-md" />}
      </button>
      ...
      <div
        ...
        className="absolute right-2 top-1/2 -translate-y-1/2 z-20 p-2 text-gray-500 hover:text-cyber-cyan cursor-help transition-colors"
